...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Also fit the plain baseline feature set and keep the LLM plan's "
            "predictions only if the plan's out-of-fold AUC is strictly better."
        ),
    )
    parser.add_argument(
        "--gate-margin",
        ...
        default=0.002,
        help=(
            "Minimum OOF AUC gain the plan must clear to be adopted. Defaults "
            "to 0.002 because replay testing showed differences below roughly "
            "this size are noise rather than signal (see LEARNINGS.md); a zero "
            "margin lets a coin-flip fluctuation ship extra features."
        ),
    )
    ...
    def evaluate(mode: str, plan: dict | None) -> dict:
        """Fit one feature configuration and return its predictions + OOF AUC.

        Never raises: a failed configuration returns a constant-prior fallback
        with cv_auc None so the caller can still fall back to the other arm.
        """
        ...
        try:
            ...
            errors.append(f"[{mode}] catboost: {error}")
            try:
                ...
                errors.append(f"[{mode}] sklearn: {fallback_error}")
        return {
            "mode": mode,
            "method": method,
            "cv_auc": cv_auc,
    ...
    gate = None
    try:
        chosen = evaluate(arguments.feature_mode, transform_plan)
    except Exception as error:
        # A malformed plan must never sink the run; fall back to no features.
        errors.append(f"[{arguments.feature_mode}] prepare_features: {error}")
        chosen = evaluate("none", None)
        chosen["mode"] = "none_after_plan_error"

    # Gate: only keep engineered/plan features when they actually earn it on
    # out-of-fold AUC. Blind transformation bundles measurably hurt in replay
    # testing (see LEARNINGS.md), so the plan must beat the plain baseline.
    if arguments.gate_baseline and chosen["mode"] not in {"none", "none_after_plan_error"}:
        try:
            baseline = evaluate("none", None)
        except Exception as error:
            errors.append(f"[none] prepare_features: {error}")
            baseline = None
        if baseline is not None:
            plan_auc = chosen["cv_auc"]
            base_auc = baseline["cv_auc"]
            if plan_auc is None or (
                base_auc is not None and plan_auc <= base_auc + arguments.gate_margin
            ):
                gate = {
                    "decision": "rejected_plan_kept_baseline",
                    "plan_cv_auc": plan_auc,
                    "baseline_cv_auc": base_auc,
                    "margin": arguments.gate_margin,
                }
                chosen = baseline
            else:
                gate = {
    ...
    write_submission(sample, test, target, chosen["predictions"], output)
    print(
        json.dumps(
            {
                "method": chosen["method"],
                "feature_mode_used": chosen["mode"],
                "cv_auc": chosen["cv_auc"],
                "gate": gate,
                "train_rows": len(train),
                "test_rows": len(test),
                "features": chosen["features"],
                "categorical_features": chosen["categorical_features"],
                "feature_metadata": chosen["feature_metadata"],
                "target": target,
                "output": str(output),
                "errors": errors,
            },
