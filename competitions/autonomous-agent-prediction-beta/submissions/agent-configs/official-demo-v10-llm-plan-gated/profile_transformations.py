...
def main() -> int:
    ...
    output = {
        ...
        "advice_policy": (
            "Your plan IS executed, but only through a fixed allowlist and only "
            "if it earns it: the runner fits your plan's feature set and the "
            "plain baseline, then keeps your plan only when its out-of-fold AUC "
            "is strictly higher. Unknown keys, unknown transforms, and unknown "
            "columns are ignored. Recommend a transform only when this profile "
            "shows a concrete skew, tail, bound, or scale reason for it — an "
            "indiscriminate bundle of transforms measured WORSE than the "
            "baseline in replay testing, so propose few and justify each."
        ),
        "allowed_plan_schema": {
