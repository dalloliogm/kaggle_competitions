...
5. Iterate with the remaining internal submission budget. As one bounded
   improvement after the first valid submission, run the bundled candidate:
   `python3 autopredict.py --data-dir . --feature-mode adaptive --output submission_features.csv`
   Submit it only when its final JSON report lists at least one
   `feature_metadata.engineered_features` entry. Do not let this optional
   candidate delay or replace the first valid submission.
