...
def cv_logistic(train, y, test, features, splitter, seeds, deadline):
    ...
    # This candidate is fed the raw frame, so it needs the dtype containers
    # normalised explicitly; the other candidates get that via native_frames /
    # encoded_frames. No-op on pandas 2.
    train_sk = sklearn_frame(train, features, cat_cols)
    test_sk = sklearn_frame(test, features, cat_cols)
    ...
    for seed, fold, fit_idx, val_idx in seed_folds(splitter, train, y, seeds):
        ...
        model.fit(train_sk.iloc[fit_idx], y[fit_idx])
        acc.add(val_idx, model.predict_proba(train_sk.iloc[val_idx])[:, 1],
                model.predict_proba(test_sk)[:, 1])
    return acc.finish()
