...
2. Read `skills/feature-engineer/scripts/run_baseline.py` if you want details,
   ...
   for categoricals), cross-validates four models (LogisticRegression,
   HistGradientBoostingClassifier, RandomForestClassifier,
   ExtraTreesClassifier — all single-threaded), and writes `submission.csv`
   from whichever single model had the best out-of-fold AUC (blending all
   four diluted the strongest model's signal in testing — prefer the winner
   over an average). Always writes a valid submission, falling back to a
   constant-prior prediction if modeling fails for any reason.
