...
## Current evidence boundaries — 2026-09-05

- The new div45 weight bracket scored .75=0.935, .80=0.938, .85=0.932.
  The older smooth sweep does not establish stability for this changed pipeline.
- The four existing validator movies are all listed in the secondary checkpoint's
  training manifest. Its test list also overlaps its train list (40 movies).
  A filename or column called "test" does not establish independence.
- Fresh movie partitions cannot undo checkpoint training exposure or sparse-label
  selection bias. Use the frozen groups for diagnostics only until provenance is audited.
- Exact official aggregation and exact graph matching are separate requirements.
  The old custom validator's correct-looking formula did not prove scorer parity.
- The corrected comparator includes divisions, raw counts, official aggregation,
  macro stability and embryo groups; malformed/partial reports fail closed.
- Exp209=0.913 versus Exp203=0.917 supports a gap/division interaction on that
  baseline. Reassess mechanisms on a changed backbone instead of applying blanket closures.
