...
| 2026-08-02 | Exp165 tiled dense relinking | Exp148 unchanged except overlapping `48 um` / `12 um` local Hungarian relinking on `6bba_05db0fb1` | Synthetic tile test passed; Kaggle v1 running | Pending | Targets the known dense-frame parent-assignment bottleneck while preserving three control movies | Inspect dense-only edge delta and exact output validation |
