...
| 2026-07-27 | Exp148 confidence-adaptive two-seed edge fusion | `notebooks/biohub-exp148-adaptive-edge-fusion.ipynb` | Static validation passed: valid notebook JSON, every code cell parses, and the fork differs from Exp144 only in experiment naming/context plus `BIOHUB_SECONDARY_LINK_MODE=adaptive`; secondary detection weight remains `0.475`. Kaggle v1 is running as `dalloliogm/biohub-exp148-adaptive-edge-fusion`. | Download and validate visible output, confirm predictions differ from Exp144, then submit immediately under the user's explicit same-day authorization. |
