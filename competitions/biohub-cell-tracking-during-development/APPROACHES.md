...
| 2026-08-01 | Exp164 bidirectional crossing repair | Exp148 plus conservative pair swaps scored with predecessor and successor trajectory context | Exact output valid; 5,842 pairs examined, 0 accepted; counts identical to Exp148 | Diagnostic only; not submitted | The four-frame rule is inactive at `1.0 um` / `0.80` gates; no evidence yet that future context changes the graph | If revisiting, correct the stale tag and run a deliberately wider calibration bracket; do not submit this version |
