...
| 2026-07-18 | Exp105 weighted gap-node interpolation | `notebooks/biohub-exp105-weighted-gap-interpolation-candidate.ipynb` | Kaggle v1 running as `dalloliogm/biohub-exp105-weighted-gap-interpolation`; adds motion-aware synthetic gap-node placement with `0.5` blend and `4.0 um` clamp on the Exp092 branch | Download/validate output, then submit if structural checks pass |
| 2026-07-18 | Exp106 density spacing gate | `notebooks/biohub-exp106-density-spacing-gate-candidate.ipynb` | Prepared locally; syntax checks passed; Exp092 branch with density gain `0.04375` plus outside-reference local-spacing gate `8.50 um` | Upload/run next |
