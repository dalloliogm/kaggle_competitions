#!/usr/bin/env python3
"""Benchmark hybrid-conservative vs current hybrid to verify improvement."""
...
HYBRID_CURRENT = BASE / "hybrid-primary2p-anchor4p" / "main.py"
HYBRID_CONSERVATIVE = BASE / "hybrid-conservative-4p" / "main.py"
...
SEEDS = [911, 1012, 1113]
OUTPUT = ROOT / "submissions" / "benchmark-hybrid-conservative-vs-current.json"
...
def main() -> None:
        ...
        for seed in SEEDS:
            for players in (2, 4):
                for seat in range(players):
                    ...
                    paths[seat] = HYBRID_CONSERVATIVE

                    print(
                        f"Running hybrid-conservative {opponent_name} {players}P seed={seed} seat={seat}..."
                    )
                    env = make(
    ...
    for game in games:
        key = f"hybrid-conservative|{game['players']}P|{game['opponent']}"
        row = summary[key]
        row["games"] += 1
