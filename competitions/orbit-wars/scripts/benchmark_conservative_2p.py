#!/usr/bin/env python3
"""Quick check: does conservative variant still work in 2P?"""
...
SEEDS = [911, 1012, 1113]
OUTPUT = ROOT / "submissions" / "benchmark-conservative-2p.json"
...
def main() -> None:
    ...
    for opponent_name, opponent in OPPONENTS.items():
        for seed in SEEDS:
            for seat in range(players):
                ...
                print(f"Running 2P {opponent_name} vs conservative seed={seed} seat={seat}...")
    ...
    for game in games:
        key = f"conservative|2P|{game['opponent']}"
        row = summary[key]
        row["games"] += 1
