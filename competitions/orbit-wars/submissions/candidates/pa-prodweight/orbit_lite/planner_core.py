...
def build_target_shortlist(
    ...
    # Production-weighted offensive preference: prefer high-production targets, not
    # just the nearest. ``prod_weight`` distance-units are "spent" per point of target
    # production, so high-prod planets surface over marginally nearer low-prod ones
    # (4P compounding-production fix). ``prod`` is a finite, non-negative [P] tensor in
    # ``proximity``'s dtype, so the value stays finite on attackable slots and the
    # ``-inf`` masking + device-stable ascending-index tie-break in ``_candidate_indices``
    # are preserved. ``prod_weight == 0.0`` reproduces the legacy ``-proximity`` exactly.
    prod_weight = float(getattr(config, "prod_weight", 0.0))
    pref = prod_weight * prod.to(proximity.dtype) - proximity
    attack_pref = torch.where(attack_mask, pref, torch.full_like(proximity, float("-inf")))
