...
class ProducerLiteConfig:
    ...
    # Production weighting of offensive target selection (build_target_shortlist):
    # offensive targets rank by ``prod_weight * target_prod - proximity`` instead of
    # pure proximity, so high-production planets surface over marginally nearer
    # low-prod ones (the 4P compounding-production fix). 0.0 = legacy pure-proximity
    # ranking, so 2P/base play is byte-for-byte unchanged.
    prod_weight: float = 0.0
    # --- scoring / greedy ------------------------------------------------
    max_waves_per_turn: int = 4
    roi_threshold: float = 1.5              # fire if score > this
    min_ships_to_launch: float = 4.0
    # Fraction of a source's safe_drain actually launched per attack. Keeps a
    # defensive reserve so freshly-captured and owned planets survive the
    # counter-attack instead of being recaptured (the 4P boom-bust failure).
    # 1.0 = legacy (send everything); 4P uses <1 to enforce capture-and-hold.
    ...
    # C_k is enemy supply reachable to the target during the flight, so the
...
def plan_lite_waves(
    *,
    ...
    sizes = (drain * float(config.commit_fraction)).view(S, 1).expand(S, T).floor()                                # [S, T]
    ...
    # Cross-wave source budget = each source's safe_drain, NOT its raw garrison.
    # With the raw garrison, a source whose safe_drain is small can still be picked
    # for several waves in one turn and drained to ~0 (the 4P boom-bust: planets
    # emptied across multiple launches, then recaptured). Capping the budget to
    # safe_drain lets a planet contribute at most what it can shed while staying
    # held over the horizon, regardless of how many targets want it.
    source_budget = obs.ships.to(dtype).clone()
    if bool(source_exists.any()):
        valid_src = source_idx[source_exists]
        source_budget[valid_src] = torch.minimum(source_budget[valid_src], drain[source_exists])
