...
2. **Neighbor-well curve transfer v2 — failed with `Notebook Timeout` (`55022483`, 2026-07-27).** The visible-run microbenchmark proved only that the added sampled-index/k=1 neighbor stage was fast at a simulated 52-well query count. It did not exercise the entire notebook at hidden-dataset scale, so the earlier claim that v2 solved grading scalability was incomplete. Do not resubmit v2 unchanged.
3. **PF-seed posterior branch hedge — submitted and pending (`55036598`, 2026-07-27), but at high timeout risk.** `dalloliogm/rogii-pf-seed-branch-hedge` v1 inherits the timed-out neighbor-transfer v2 notebook and adds PF-seed statistics. The clean next implementation should instead start from the grading-proven continuity-fade/no-override runtime (7.111 and 7.112), retain the already-computed 128 PF trajectories only long enough to calculate the two-mode statistics, apply the bounded hedge, and omit neighbor transfer and its benchmark entirely.
