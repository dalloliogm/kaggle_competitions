...
2. **Neighbor-well curve transfer** for wells with a close (<150 ft) spatial neighbor — **now the top priority.** Reported as *the* dominant lever separating top scores from this pipeline family, and unlike bimodal-hedge, the reported magnitude (line-oracle 6.70 vs. far-well 10.15, a >3 ft gap) is large enough to plausibly stand out above the ~0.5-1.5 ft noise floor we've now characterized. Needs new feature engineering (find nearest same-frame neighbor among train/real-test wells, transfer its interpreted TVT curve/shape, not just a trend line) — a bigger engineering effort than flag-flipping, worth scoping properly before submitting.
