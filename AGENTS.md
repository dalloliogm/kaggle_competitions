...
- **Run the Kaggle CLI via `uvx`** — it is not installed in this repo's env: `uvx --index-url https://pypi.org/simple kaggle <command>` (the default package index is a private mirror that returns 401). Credentials live at `~/.kaggle/kaggle.json`. Useful subcommands: `competitions submit | submissions | episodes <submission_id> | replay <episode_id> | leaderboard`.
